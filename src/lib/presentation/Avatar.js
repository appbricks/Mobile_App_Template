 ...
 * 
 * The avatar class manages the lifecycle of a user's avatar image. 
 */
...
export default class Avatar {
  ...
  constructor() {
    this.image = new MutableImage(null);
  ...
  /**
   * Updates this avatar instance with the given user. It
   * will first check for a profile picture url within the
   * user instance. If one is not found then it will check
   * on gplus and if not found there it will check gravatar.
   * 
   * @param {*} user  The user to determin the avatar image url for
   */
  updateAvatar(user) {

    const _notifyAvatarUpdate = this._notifyAvatarUpdate.bind(this);

    if (user.profilePictureUrl) {
      _notifyAvatarUpdate(user, user.profilePictureUrl);
    } else {
      ...
      if (emailAddress && emailAddress.length > 0) {
        ...
        gplusUrl = gplusProfilePicture(
          url => {
            ...
            if (!url) {
              ...
              // Check for a profile picture 
              // is hosted on gravatar
              gravatar(
                url => {
                  _notifyAvatarUpdate(user, url);
                },
                emailAddress, { size: 320 }
              );
        ...
        _notifyAvatarUpdate(user, null);
      }
    }
  }

  /**
   * Checks if avatar url / title has changed and
   * notifies the associated avata image instance 
   * of the change.
   * 
   * @param {*} user 
   * @param {*} avatarUrl 
   */
  _notifyAvatarUpdate(user, avatarUrl) {
    ...
    if (avatarUrl) {
      ...
      if (user.firstName && user.firstName.length > 0
        && user.familyName && user.familyName.length > 0) {

        avatarTitle = (user.firstName.charAt(0) + user.familyName.charAt(0))
          .toUpperCase();
    ...
    if (this.avatarUrl != avatarUrl ||
      ...
      this.image.setUri(avatarUrl, avatarTitle);
    }
  }
}

/**
 * Returns a link to a google profile image
 * 
 * @param {*} cb 
 * @param {*} email 
 */
export function gplusProfilePicture(cb, email) {

  const url = "https://picasaweb.google.com/data/entry/api/user/" + email + "?alt=json";
  Logger.trace("avatar", "retrieving gplus profile from", url);

  fetch(url)
    .then(response => response.json())
    .then(responseJSON => {

      Logger.trace("avatar", "gplus profile", responseJSON);

      let thumbnail = responseJSON.entry["gphoto$thumbnail"];
      if (thumbnail && thumbnail["$t"]) {
        let photoUrl = thumbnail["$t"];
        photoUrl = photoUrl.replace(/s64-c\/.*/, "");

        fetch(photoUrl)
          .then(response => {
            let contentLength = response.headers.get("content-length");
            if (contentLength && parseInt(contentLength) > 2506) {
              cb(photoUrl);
            } else {
              Logger.trace("avatar", "no gplus profile picture found");
              cb(null);
            }
          })
          .catch(error => {
            Logger.trace("avatar", "unable to retrieve gplus profile picture", error);
            cb(null);
          });
...
/**
 * Returns a link to a gravatar given an email.
 * 
 ...
 * @param {*} options
 */
export function gravatar(cb, email, options) {
  ...
  fetch("https://www.gravatar.com/" + emailHash + ".json")
    .then(response => {
      if (response.ok) {
        cb("https://www.gravatar.com/avatar/" + emailHash + "?" + params.join("&"));
      } else {
        cb(null);
      }
    })
    .catch(error => {
      Logger.trace("avatar", "unable to retrieve gravatar user profile", error);
      cb(null);
    });
